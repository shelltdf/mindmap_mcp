...
- **画布左上角悬浮**显示**快捷键**面板（`canvas_wrap` 内**最后一个子节点** + 较高 `z-index`，叠在 jsMind 与缩放控件之上，避免占位导致画布变形）：带标题栏（**Shortcuts** / **快捷键**），**默认收起**，**点击标题按钮**由脚本切换 `collapsed` 展开/收起（避免 Webview 内 `<details>` 无法展开）；列表为**白色文字**、半透明深色底，**每条快捷键一行**；分为 **「选中对象后」** 与 **「无需选中」** 两组（随 **Language** 菜单中英文切换）。展开后**正文区域不拦截画布操作**（点击穿透至画布）。
- 在 xmind 风格交互中，结构级增删/移动入口已隐藏或禁用；你仍可通过标题编辑与展开/折叠等操作，尽可能保持与 xmind 一致的交互体验
- 中键拖拽：平移画布
- 滚轮缩放：缩放画布（以**当前视口中心**为锚）
- 左下角缩放区：**适应** = 全图入窗；**根节点** = 将根主题移到视口中心；**还原** = 仅将缩放回到 **100%**（以视口中心为锚，**不**为对齐根节点而平移整图）；**双击百分比** 与右键「还原缩放比例」同「还原」
- 双击节点：按 jsMind 默认方式**内联编辑**节点标题（非 `window.prompt`）
- `Enter`：在选中节点同级创建兄弟节点（根节点除外）
- `Tab`：在选中节点下创建子节点
- 新建节点（上述操作、双击空白、粘贴子树、嵌入子节点等）后，画布会**自动平移**使新节点保持在可视区内（不改变当前缩放比例；特大节点会尽量居中）
...
- **客户区尺寸变化**（如全屏、拖宽窗口）：外层平移会对 `#canvasWrap` 尺寸变化做 **Δ/2** 补偿，尽量保持**视口中心**对准的内容不变（与滚轮缩放以中心为锚一致）。
