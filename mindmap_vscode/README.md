...
**未保存到磁盘的脑图**（内容只在 Webview 里、尚未 Save / Save As）无法从工作区当普通文件读写；要批量加节点请调用 `mindmapVscode.aiApplyOps`，或先保存成 `.jm` / `.mmd` 再改文件。链式多条 `add`（后一步的 `parentId` 依赖前一步新建的节点）不要用 `dryRun: true` 整批预演，否则父节点不存在会报错。根节点 `id` 由运行时分配（空白/新建多为 `r_` 前缀），**不要**写死 `root`；构造 `ops` 前请先 `aiGetTree` 取当前根与子节点实际 id。
