...
- **分割条拖动与 Dock 宽度（`webview-app.js`）**：布局为 **画布 | `#mainRowSplitter` | 右 Dock**；**向右拖分割条**须**缩小** Dock（边界随鼠标右移、画布变宽）。故 **`dx = clientX - startX`** 时 **LTR**（`#mainRow` 计算样式 `direction` 非 `rtl`）用 **`startW - dx`**；**RTL** 下 flex 视觉为 **Dock | 分割条 | 画布**，用 **`startW + dx`**。
- **`#dockRightStack` 行内布局**：`#dockRightStack` 由 **`applyDockWidth`** 设**固定总宽**；其与 **`#dockFoldStrip`** 之间为横向 flex，默认 **`justify-content: flex-start`**。若并列的 **`.mm-dock-view`** 使用 **`flex-grow: 0`**，则「总宽 − 内容自然宽 − 缘条 28px」会留在 **`#dockFoldStrip` 右侧**，窗右缘出现**大块空白**。**`.mm-dock-view`** 须 **`flex: 1 1 auto`** 且 **`min-width: 0`**，由 **Dock View 列占满剩余宽度**，**`#dockFoldStrip` 贴紧 Dock Area 右缘**（与 `panel.ts` 一致）。
- **Dock 内容区横向撑满**：**`.dock-right`** 在 **`.mm-dock-view`** 内须 **`width: 100%` / `align-self: stretch`**；**`.dock-display`** 须 **`flex: 1 1 auto; width: 100%; max-width: 100%`**（**勿**固定内容宽如历史 **`208px`**），以便外侧拖宽 Dock Area 后 **Format / Icon / Theme** 面板在横向上占满 **Dock View**。
